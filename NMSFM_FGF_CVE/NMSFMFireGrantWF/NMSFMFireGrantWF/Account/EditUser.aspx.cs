...
accountService = new AccountService(userWebModel, logger);
        ...
        protected async void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!Page.IsPostBack)
                {
                    ...
                    if (help != null)
                    {
                        helpdiv.InnerHtml = help.HelpText + "<hr />" + help.HelpText;
                    }
                ...
                if (webUser != null)
                {
                    ...
                    else
                    {
                        ...
                        if (inspector != null) //Internal User with matching login was found
                        {
                            ...
                            chkGrantAdmin.Checked = webUser.NMFGC;
                            chkReadOnly.Checked = webUser.Readonly;
                        }
                        else
                        {
        ...
        private async Task<bool> LoadAllDepartments()
        {
            try
            {
        ...
        private async void LoadAssociatedDepartments()
        {
            try
            {
                ...
                List<DetailedFGDepartment> initialDepts = new List<DetailedFGDepartment>();
                List<DetailedFGDepartment> removallDepts = new List<DetailedFGDepartment>();
                ...
                foreach (v_AddressParties party in addresses)
                {
                    ...
                    initialDepts.Add(thisDept);
                }
                ViewState["assDepts"] = assDepts;
                ViewState["initialDepts"] = assDepts;
                ViewState["removalDepts"] = removallDepts;
        ...
        protected async void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dvDepartments.Visible == true && assDepts.Count < 1)
                {
                    ...
                    if (duplicate != null)
                    {
                        ...
                        if (userUpdated)
                        {
                            if (rbParties.Checked)
                            {
                                ...
                                model.PartyID = codePalId;
                                model.PartyName = txtFirstName.Text;
                                model.Email = txtEmail.Text;
                                model.Phone = txtPhone.Text;
                                await partyService.UpdateParty(model);
                                ...
                                List<DetailedFGDepartment> removalDepts = (List<DetailedFGDepartment>)ViewState["removalDepts"];
                                if (removalDepts.Count > 0)
                                {
                                    foreach (DetailedFGDepartment remDept in removalDepts)
                                    {
                                        var removalParty = await partyService.GetAddressPartyByIdAsync(codePalId, new Guid(remDept.addressId.ToString()));
                                        await partyService.RemoveAddressParty(removalParty);
                                    }
                                }
                            }
                            else
                            {
                                //Update Inspector (Should not allow system to update user information)?
                                ...
                                //await systemService.UpdateInspector(inspector);
        ...
        protected void rgDepartments_ItemCommand(object sender, GridCommandEventArgs e)
        {
            try
            {
                ...
                if (e.Item is GridDataItem)
                {
                    ...
                    {
                        List<DetailedFGDepartment> assDepts = (List<DetailedFGDepartment>)ViewState["assDepts"];
                        List<DetailedFGDepartment> initialDepts = (List<DetailedFGDepartment>)ViewState["initialDepts"];
                        List<DetailedFGDepartment> removalDepts = (List<DetailedFGDepartment>)ViewState["removalDepts"];
                        ...
                        foreach (DetailedFGDepartment dept in initialDepts)
                        {
                            if (dept.addressId == new Guid(id))
                            {
                                removalDepts.Add(dept);
                            }
                        }
                        ViewState["assDepts"] = assDepts;
                        ViewState["removalDepts"] = removalDepts;
                        rgDepartments.DataSource = assDepts;
                        rgDepartments.Rebind();
                        rgDepartments.Focus();
                    }
