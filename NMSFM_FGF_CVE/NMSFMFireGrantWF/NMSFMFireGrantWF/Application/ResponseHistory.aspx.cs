...
// private IAccountService accountService; // legacy field, currently unused
        ...
        private void LoadResponseHistory(FG_App_ResponseHistory model)
        {
            try
            {
                ...
                if (model.NERISCurrent == 1) { rbNERISCurrentYes.Checked = true; }
                if (model.NERISCurrent == 2) { rbNERISCurrentNo.Checked = true; }
        ...
        private async Task<bool> SaveForm()
        {
            try
            {
                ...
                bool isValid = true;
                int nfirsCurrent = 0;
                if (rbNERISCurrentYes.Checked) { nfirsCurrent = 1; }
                if (rbNERISCurrentNo.Checked) { nfirsCurrent = 2; }
                if (nfirsCurrent == 0)
                {
                    errorMessage += "NERIS Current reponse is required<br />";
                    isValid = false;
                }

                //if (Convert.ToInt32(txtTotal.DbValue) < 1)
                //{
                //    errorMessage += "Total Calls must be greater than zero<br />";
                //    isValid = false;
                //}
                ...
                model.NERISCurrent = nfirsCurrent;
