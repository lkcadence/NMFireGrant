...
// private IAccountService accountService; // legacy field, currently unused
        ...
        private async void LoadEquipmentNeeds(DetailedFGAppEquipmentNeeds model)
        {
            try
            {
                ...
                if (model.ApplicationEquipment.Count > 0)
                {
                    string priorityNum = ddlPriorityCategory.Items.FindByText(model.ApplicationEquipment[0].PriorityCategory).Value;
                    ddlPriorityCategory.SelectedValue = priorityNum;
                    ddlPriorityCategory.Enabled = false;
                    await LoadEquipment(Convert.ToInt16(priorityNum));
                }
                //end add
            }
            catch (Exception ex)
        ...
        private async Task<bool> SaveForm()
        {
            try
            {
                ...
                //if (equipment == null || equipment.Count < 1)
                //{
        ...
        protected async void btnDeleteEquipment_ServerClick(object sender, EventArgs e)
        {
            try
            {
                ...
                int num = 1;
                foreach (FG_App_ApplicationEquipment item in applicationEquipment)
                {
                    item.Number = num;
                    num += 1;
                }
                ...
                //added 12/21/2023 (vwd)
                if (applicationEquipment.Count > 0)
                {
                    string priorityNum = ddlPriorityCategory.Items.FindByText(applicationEquipment[0].PriorityCategory).Value;
                    ddlPriorityCategory.SelectedValue = priorityNum;
                    await LoadEquipment(Convert.ToInt16(priorityNum));
                    ddlPriorityCategory.Enabled = false;
                }
                else
                {
                    ...
                    await LoadEquipment();
                    ddlPriorityCategory.Enabled = true;
                }
                //end add
        ...
        protected void btnSaveEquipment_ServerClick(object sender, EventArgs e)
        {
            try
            {
                ...
                equipment.Number = applicationEquipment.Count + 1;
