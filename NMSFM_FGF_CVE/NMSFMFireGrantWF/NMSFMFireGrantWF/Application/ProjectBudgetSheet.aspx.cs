...
// private IAccountService accountService; // legacy field, currently unused
        ...
        private async Task<bool> SaveForm()
        {
            try
            {
                ...
                decimal totalAmountRequested = 0;
                decimal totalStipendAmount = 0;
                decimal equipmentCost = Convert.ToDecimal(hfEquipmentCost.Value);
                decimal onePenny = Convert.ToDecimal(.01);
                ...
                if (txtTotalAmount.Text != "")
                {
                    ...
                    {
                        errorMessage += "Total Project Amount is Required (enter zero if not requested).<br />";
                        isValid = false;
                    }
                }
                else
                ...
                if (txtAmountRequested.Text != "")
                {
                    try
                    {
                        totalAmountRequested = Convert.ToDecimal(txtAmountRequested.DbValue);
                    }
                    catch
                    {
                        errorMessage += "Total Amount Requested is Required (enter zero if not requested).<br />";
                        isValid = false;
                    }
                }
                else
                ...
                if (txtStipendAmountRequested.Text != "")
                {
                    try
                    {
                        totalStipendAmount = Convert.ToDecimal(txtStipendAmountRequested.DbValue);
                    }
                    catch
                    {
                        errorMessage += "Total Amount Requested is Required (enter zero if not requested).<br />";
                ...
                {
                    errorMessage += "Total Stipend Amount is Required (enter zero if not requested).<br />";
                    isValid = false;
                }

                if (totalProjectCost < onePenny)
                {
                    if (totalStipendAmount < onePenny)
                    {
                        errorMessage += "Total Project Amount or Stipend Amount is Required.<br />";
                        isValid = false;
                    }
                    if (totalProjectCost < equipmentCost)
                    {
                        errorMessage += "Total Project Amount cannot be zero if requesting equpment in the Equipment Needs section.<br />";
                        isValid = false;
                    }
                }
                else
                {
                    if (totalAmountRequested < onePenny)
                    {
                        errorMessage += "Amount Requested is Required.<br />";
                        isValid = false;
                    }
                    if (totalAmountRequested > totalProjectCost)
                    {
                        errorMessage += "Amount Requested cannot be greater than the Total Project Cost.<br />";
                        isValid = false;
                    }
                    if (equipmentCost < onePenny)
                    {
                        errorMessage += "If total project cost > 0, please identify equipment to be purchased in the Equipment Needs Section.<br />";
                        isValid = false;
                    }
                    if (totalProjectCost < equipmentCost)
                    {
                        errorMessage += "Total Project Amount must be greater than or equal to the total equipment cost.<br />";
                        isValid = false;
                    }
                }
